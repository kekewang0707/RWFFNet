...
class RWFFNet(nn.Module):

    def __init__(self, num_classes=10):
        super(RWFFNet, self).__init__()
        self.backbone = resnet.resnet18(pretrained=False)
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.cbp = CompactBilinearPooling(512, 512, 8000)

        self.alpha = nn.Sequential(nn.Linear(8000, 1),
                                   nn.Sigmoid())
        self.beta = nn.Sequential(nn.Linear(16000, 1),
                                  nn.Sigmoid())
        self.fc = nn.Linear(16000, num_classes)
    ...
    def forward(self, x, crop):
        ...
        alphas = []
        org = self.backbone(x)
        org = self.cbp(org, org)
        vs.append(org)
        alphas_org = self.alpha(org)
        alphas.append(alphas_org)
        ...
        output = vs_stack_16000.mul(betas_stack * alphas_stack).sum(2).div((betas_stack * alphas_stack).sum(2))
...
if __name__ == '__main__':
    ...
    corp = torch.randn(1, 5, 3, 224, 224)
    model = RWFFNet(10)
    model(x, corp)
